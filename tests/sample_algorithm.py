...
class Algo(AlgoFramework):
    ...
    def period(self):
        ...
        """
        return SymbolRequestPeriod.YEAR
                ...
                if len(symbolContext.ema_25) >= gradientCount:
                    ...
                    if len(context.openPositions) == 0 or not any(x.symbol == self.symbol for x in context.openPositions):
                        ...
                        if gradSMA_10 > 0 and gradSMA_25 > 0: # and rsi_ema[-1] < 50:
                            context.openPosition(PositionDirection.LONG, quote, 5, stopLoss=quote.low - 1000, stopType=PositionStopType.TRAILING)
                        elif gradSMA_10 < 0 and gradSMA_25 < 0: # and rsi_ema[-1] > 50:
                            context.openPosition(PositionDirection.SHORT, quote, 5, stopLoss=quote.high + 1000, stopType=PositionStopType.TRAILING)
